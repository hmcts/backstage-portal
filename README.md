...
It can be confusing having both a app-config.yaml and app-config.production.yaml and a future task should be carried out by a HMCTS dev to look at consolidating 
these into one file. Sandbox and PTL use the app-config.production.yaml configuration file, whereas local development uses the app-config.yaml file. 
When removing one or the other the build currently fails.
